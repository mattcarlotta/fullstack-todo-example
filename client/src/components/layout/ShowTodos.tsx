...
export default function ShowTodos(props: ShowTodoProps) {
    ...
    const handleCompletedTodo = async ({ id }: TodoId) => {
        try {
            const res = await client.completeTodo.query({ id, token: token() });
            dispatchToastEvent({ type: 'success', message: res.message });
            handleUpdateTodos(res.todos);
        } catch (error: any) {
    ...
    return (
        <Show
            when={todos().length}
            fallback={<AddTodoForm token={token()} handleUpdateTodos={handleUpdateTodos} />}
        >
